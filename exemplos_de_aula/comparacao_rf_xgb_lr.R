...
credit_xgb_recipe <- recipe(Status ~ ., data = credit_train) |>
  step_impute_linear(Income, Assets, Debt, impute_with = imp_vars(Expenses)) %>%
  step_impute_mode(all_nominal_predictors()) %>%
  step_novel(all_nominal_predictors()) |>
  step_zv(all_predictors()) |>
...
credit_xgb_wf <- workflow() %>%
  add_model(credit_xgb_model) %>%
  add_recipe(credit_xgb_recipe)
...
credit_xgb_tune_grid <- tune_grid(
  credit_xgb_wf,
  resamples = credit_resamples,
  grid = grid_xgb,
  control = control_grid(verbose = TRUE),
...
credit_xgb_best_params <- select_best(credit_xgb_tune_grid, "roc_auc")
credit_xgb_wf <- credit_xgb_wf %>% finalize_workflow(credit_xgb_best_params)
credit_xgb_last_fit <- last_fit(credit_xgb_wf, credit_initial_split)
...
credit_test_preds <- bind_rows(
  ...
  collect_predictions(credit_xgb_last_fit) %>% mutate(modelo = "xgb")
)
...
credit_xgb_last_fit_model <- credit_xgb_last_fit$.workflow[[1]]$fit$fit
vip(credit_xgb_last_fit_model)
